...
function World( options ) {
  ...
  this.objects = new Dequeue( options.objectCapacity )
...
World.prototype = {
  ...
  interpolate: function( dt ) {
    ...
    for( var i = 0; i < this.objects.length; i++ ) {
      ...
      this.objects[i].previousPosition.lerp(
        this.objects[i].position, t,
        this.objects[i].interpolatedPosition
      )

      this.objects[i].previousQuaternion.slerp(
        this.objects[i].quaternion, t,
        this.objects[i].interpolatedQuaternion
      )

      this.objects[i].previousQuaternion.normalize()
